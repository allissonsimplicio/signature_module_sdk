...
export class SignerService {
   ...
   */
  async create(envelopeId: string, dto: SignerInput): Promise<Signer> {
    // ✅ v3.0: Direct type (no ApiResponse wrapper)
    const response = await this.http.post<Signer>(
      `/api/v1/envelopes/${envelopeId}/signers`,
      dto
    );
   ...
   */
  async findAll(filters?: SignerFilters): Promise<PaginatedResponse<Signer>> {
    // ✅ v3.0: Direct type (no ApiResponse wrapper)
    const response = await this.http.get<PaginatedResponse<Signer>>('/api/v1/signers', {
      params: filters,
    });
    return response.data;
  }
   ...
   */
  async findByEnvelope(
    envelopeId: string,
    filters?: Omit<SignerFilters, 'envelopeId'>
  ): Promise<Signer[]> {
    const response = await this.http.get<Signer[]>(
      `/api/v1/envelopes/${envelopeId}/signers`,
      { params: filters }
    );
    return response.data;
   ...
   */
  async findById(id: string): Promise<Signer> {
    // ✅ v3.0: Direct type (no ApiResponse wrapper)
    const response = await this.http.get<Signer>(`/api/v1/signers/${id}`);
    return response.data;
  }
   ...
   */
  async update(id: string, dto: Partial<SignerInput>): Promise<Signer> {
    // ✅ v3.0: Direct type (no ApiResponse wrapper)
    const response = await this.http.put<Signer>(`/api/v1/signers/${id}`, dto);
    return response.data;
  }
   ...
   */
  async notify(envelopeId: string): Promise<void> {
   ...
   */
  async getSigningUrl(id: string): Promise<SigningUrlResponse> {
    // ✅ v3.0: Direct type (no ApiResponse wrapper)
    const response = await this.http.get<SigningUrlResponse>(
      `/api/v1/signers/${id}/signing-url`
    );
    return response.data;
  }
   ...
   */
  async refreshSignerToken(refreshToken: string): Promise<TokenPairResponse> {
    // ✅ v3.0: Direct type (no ApiResponse wrapper)
    const response = await this.http.post<TokenPairResponse>(
      '/api/v1/signers/refresh-token',
      { refreshToken }
    );
    return response.data;
  }
   ...
   */
  async revokeSignerToken(refreshToken: string): Promise<RevokeTokenResponse> {
    // ✅ v3.0: Direct type (no ApiResponse wrapper)
    const response = await this.http.post<RevokeTokenResponse>(
      '/api/v1/signers/revoke-token',
      { refreshToken }
    );
    return response.data;
  }
   ...
   */
  async startAuthentication(id: string): Promise<StartAuthenticationResponse> {
    // ✅ v3.0: Direct type (no ApiResponse wrapper)
    const response = await this.http.post<StartAuthenticationResponse>(
      `/api/v1/signers/${id}/authenticate/start`
    );
    return response.data;
  }
  ...
  async addQualificationRequirement(
    documentId: string,
    dto: AddQualificationRequirementInput
  ): Promise<QualificationRequirement> {
    // ✅ v3.0: Direct type (no ApiResponse wrapper)
    const response = await this.http.post<QualificationRequirement>(
      `/api/v1/documents/${documentId}/qualification-requirements`,
      dto
    );
    return response.data;
  }
   ...
   */
  async uploadSignature(signerId: string, file: Buffer | Blob): Promise<Signer> {
    ...
    // ✅ v3.0: Direct type (no ApiResponse wrapper)
    const response = await this.http.post<Signer>(
      `/api/v1/signers/${signerId}/signature`,
      formData,
      {
        headers: {
   ...
   * @param signerId - ID do signatário
   */
  async deleteSignature(signerId: string): Promise<void> {
   ...
   */
  async uploadInitial(signerId: string, file: Buffer | Blob): Promise<Signer> {
    const formData = new FormData();
    ...
    // ✅ v3.0: Direct type (no ApiResponse wrapper)
    const response = await this.http.post<Signer>(
      `/api/v1/signers/${signerId}/initial`,
      formData,
      {
        headers: {
          'Content-Type': 'multipart/form-data',
   ...
   * @param signerId - ID do signatário
   */
  async deleteInitial(signerId: string): Promise<void> {
   ...
   * ```
   */
  async getSigningSession(): Promise<SigningSessionResponse> {
    // ✅ v3.0.1: Endpoint agregado para contexto de assinatura
    const response = await this.http.get<SigningSessionResponse>('/api/v1/signing-session');
    return response.data;
  }
}
