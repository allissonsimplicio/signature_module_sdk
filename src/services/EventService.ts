...
export class EventService {
   ...
   * ```
   */
  async findAll(filters?: EventFilters): Promise<PaginatedResponse<ApiEvent>> {
    const response = await this.http.get<PaginatedResponse<ApiEvent>>(
      '/api/v1/events',
      { params: filters }
    );
  ...
  async findByEnvelope(
    envelopeId: string,
    filters?: Omit<EventFilters, 'envelopeId'>
  ): Promise<PaginatedResponse<ApiEvent>> {
    const response = await this.http.get<PaginatedResponse<ApiEvent>>(
      `/api/v1/envelopes/${envelopeId}/events`,
      { params: filters }
    );
    return response.data;
  }
   ...
   * ```
   */
  async findById(eventId: string): Promise<ApiEvent> {
    const response = await this.http.get<ApiEvent>(`/api/v1/events/${eventId}`);
    return response.data;
  }
   ...
   * ```
   */
  async getStats(filters?: Partial<EventFilters>): Promise<EventStats> {
    const response = await this.http.get<EventStats>(
      '/api/v1/events/stats',
      { params: filters }
    );
    return response.data;
   ...
   * ```
   */
  async getActivitySummary(date: string): Promise<EventActivitySummary> {
    const response = await this.http.get<EventActivitySummary>(
      `/api/v1/events/activity/${date}`
    );
    return response.data;
  }
  ...
  async findByDocument(
    documentId: string,
    filters?: Omit<EventFilters, 'documentId'>
  ): Promise<PaginatedResponse<ApiEvent>> {
    const response = await this.http.get<PaginatedResponse<ApiEvent>>(
      `/api/v1/events`,
      { params: { ...filters, documentId: documentId } }
    );
    return response.data;
  }
  ...
  async findBySigner(
    signerId: string,
    filters?: Omit<EventFilters, 'signerId'>
  ): Promise<PaginatedResponse<ApiEvent>> {
    const response = await this.http.get<PaginatedResponse<ApiEvent>>(
      `/api/v1/events`,
      { params: { ...filters, signerId: signerId } }
    );
    return response.data;
  }
  ...
  async findByTemplate(
    templateId: string,
    filters?: Omit<EventFilters, 'templateId'>
  ): Promise<PaginatedResponse<ApiEvent>> {
    const response = await this.http.get<PaginatedResponse<ApiEvent>>(
      `/api/v1/events`,
      { params: { ...filters, templateId: templateId } }
    );
    return response.data;
  }
