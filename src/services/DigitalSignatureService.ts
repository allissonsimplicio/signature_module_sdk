...
import { validateCertificateFile } from '../validators';
...
export class DigitalSignatureService {
  ...
  async uploadCertificate(
    certificateFile: Buffer | File | Blob,
    password: string,
    options?: {
      passwordHint?: string;
   ...
   * ```
   */
  async listCertificates(filters?: CertificateFilters): Promise<DigitalCertificate[]> {
    try {
      const params: any = {};
      ...
      const response = await this.client.get<DigitalCertificate[]>('/digital-signatures/certificates', {
        params,
      });
      ...
      if (axios.isAxiosError(error) && error.response) {
        throw new ApiError(
          error.response.data.message || 'Failed to list certificates',
          error.response.status,
          error.response.statusText,
        );
      }
   ...
   * ```
   */
  async getCertificate(certificateId: string): Promise<DigitalCertificate> {
    try {
      const response = await this.client.get<DigitalCertificate>(
        `/digital-signatures/certificates/${certificateId}`,
      );
      return response.data;
    } catch (error) {
      if (axios.isAxiosError(error) && error.response) {
        throw new ApiError(
          error.response.data.message || 'Failed to get certificate',
          error.response.status,
          error.response.statusText,
        );
      }
  ...
  async getCertificateStats(): Promise<CertificateStats> {
    try {
      const response = await this.client.get<CertificateStats>('/digital-signatures/certificates/stats');
      return response.data;
    } catch (error) {
      if (axios.isAxiosError(error) && error.response) {
        throw new ApiError(
          error.response.data.message || 'Failed to get certificate stats',
          error.response.status,
          error.response.statusText,
        );
      }
   ...
   * ```
   */
  async activateCertificate(certificateId: string): Promise<void> {
    try {
      await this.client.patch(`/digital-signatures/certificates/${certificateId}/activate`);
    } catch (error) {
      if (axios.isAxiosError(error) && error.response) {
        throw new ApiError(
          error.response.data.message || 'Failed to activate certificate',
          error.response.status,
          error.response.statusText,
        );
      }
  ...
  async deactivateCertificate(certificateId: string): Promise<void> {
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        throw new ApiError(
          error.response.data.message || 'Failed to deactivate certificate',
          error.response.status,
          error.response.statusText,
        );
      }
  ...
  async revokeCertificate(certificateId: string, reason: string): Promise<void> {
    try {
      const payload: RevokeCertificateDto = { reason };
