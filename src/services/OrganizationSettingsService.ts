...
import { validateLetterheadFile, validateLogoFile } from '../validators';
...
export class OrganizationSettingsService {
   ...
   * ```
   */
  async update(data: UpdateOrganizationSettingsDto): Promise<OrganizationSettings> {
    const response = await this.httpClient.put<OrganizationSettings>(
      '/api/v1/organization-settings',
      data
    );
    return response.data;
  ...
  async uploadLetterhead(
    letterheadFile: Buffer | Blob | File,
    options: UploadLetterheadOptions = {}
  ): Promise<UploadLetterheadResponse> {
    // Validar arquivo (MIME type PNG e file size 10MB)
    validateLetterheadFile(letterheadFile);
    ...
    if (letterheadFile instanceof Buffer) {
      ...
      const arrayBuffer = letterheadFile.buffer.slice(
        letterheadFile.byteOffset,
        letterheadFile.byteOffset + letterheadFile.byteLength
      ) as ArrayBuffer;
      const blob = new Blob([arrayBuffer], { type: 'image/png' });
      formData.append('letterhead', blob, 'letterhead.png');
    } else {
      // Browser environment
      formData.append('letterhead', letterheadFile as Blob, 'letterhead.png');
    }
    ...
    const response = await this.httpClient.post<UploadLetterheadResponse>(
      '/api/v1/organization-settings/letterhead',
      formData,
      {
        headers: {
   ...
   * ```
   */
  async downloadLetterhead(): Promise<Blob> {
   ...
   * ```
   */
  async hasLetterhead(): Promise<boolean> {
    try {
  ...
  async uploadLogo(
    ...
    if (logoFile instanceof Buffer) {
      // Node.js environment - convert Buffer to Blob using ArrayBuffer
      const arrayBuffer = logoFile.buffer.slice(
        logoFile.byteOffset,
        logoFile.byteOffset + logoFile.byteLength
      ) as ArrayBuffer;
      ...
      // Browser environment
      formData.append('logo', logoFile as Blob, 'logo');
    }
    ...
    const response = await this.httpClient.post<UploadLogoResponse>(
      '/api/v1/organization-settings/logo',
      formData,
      {
        headers: {
          'Content-Type': 'multipart/form-data',
   ...
   * ```
   */
  async downloadLogo(): Promise<Blob> {
    const response = await this.httpClient.get<Blob>(
      '/api/v1/organization-settings/logo',
      {
        responseType: 'blob',
   ...
   * ```
   */
  async getAuthenticationLevel(): Promise<AuthenticationLevel> {
    const settings = await this.get();
